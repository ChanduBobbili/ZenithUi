...
Pass `animationType` to `useThemeMode`. These are the exact string values supported:

```tsx
import { useThemeMode } from "@zenithui/utils"

export function GlitchToggle() {
  const { ref, toggleSwitchTheme } = useThemeMode({
...
Quick mapping (UI label → `animationType`):
