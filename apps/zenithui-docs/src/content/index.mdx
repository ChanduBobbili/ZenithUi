...
A Floating Action Button (FAB) with optional speed-dial content and a full menu API.

- Configurable screen position (corners and edges).
- Compound components: Group, Label, Separator, Item, CheckboxItem, RadioGroup, RadioItem.
- Accessible (ARIA, keyboard, focus).
- Stable position with transform/scale; SSR-safe.
