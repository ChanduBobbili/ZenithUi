...
Splits a string into an array of words using a provided separator.

```ts
import { splitWord } from "@zenithui/utils"

splitWord("a,b,c", ",") // ["a", "b", "c"]
```

### convertToTitleCase

Converts a string to title case (first letter uppercased, the rest lowercased).

```ts
import { convertToTitleCase } from "@zenithui/utils"

convertToTitleCase("hELLo") // "Hello"
```

### formatNumber

Formats a number into a compact, human-readable string with suffixes (k, M, B, T).

```ts
import { formatNumber } from "@zenithui/utils"
...
Computes the percentage change between two numbers. Returns `null` when the past value is `0` (or either value is not a valid number).

```ts
import { computeDeltaPercent } from "@zenithui/utils"

computeDeltaPercent(120, 100) // 20
computeDeltaPercent(100, 0) // null
```

### formatDelta

Formats a delta percentage (from `computeDeltaPercent`) as a signed percent string.

```ts
import { computeDeltaPercent, formatDelta } from "@zenithui/utils"

const delta = computeDeltaPercent(120, 100)
formatDelta(delta) // "+20%"
```

### getDifferences

Returns the changed keys between two objects as `{ oldRecord, newRecord }`.

```ts
import { getDifferences } from "@zenithui/utils"

const { oldRecord, newRecord } = getDifferences(
  { a: 1, b: 2 },
  { a: 1, b: 3 },
)
// oldRecord = { b: 2 }
...
Persists a value in `localStorage` and returns a tuple of `[value, setValue]`. Values are JSON-serialized, and updates from other tabs/windows are observed via the `storage` event.

```tsx
import { useLocalStorage } from "@zenithui/utils"
...
Tracks an element’s size using `ResizeObserver`. Returns `{ ref, size }` where `size` is `{ width, height }`.
...
function Card() {
  ...
  return (
    <div ref={ref} style={{ resize: "both", overflow: "auto", padding: 12, border: "1px solid #ddd" }}>
      <p>
...
Subscribes to a Server-Sent Events (SSE) endpoint and returns an `EventSource` instance (or `null` during SSR). You can optionally provide message and error callbacks.

```tsx
import { useSSE } from "@zenithui/utils"

type PriceEvent = { symbol: string; price: number }
...
Provides animated light/dark theme toggling using the View Transition API when available. Returns `{ ref, toggleSwitchTheme, isDarkMode }`.

```tsx
import { useThemeMode } from "@zenithui/utils"

function ThemeToggleButton() {
  const { ref, toggleSwitchTheme, isDarkMode } = useThemeMode({ animationType: "CIRCLE" })
...
For a full prop reference and animation demos, see the Theme Toggle docs.
